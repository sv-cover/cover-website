...
class DataIterPhoto extends DataIter
{
    ...
    {
        return path_concat(get_config_value('app.photos_dir'), $this->get('filepath'));
    }
    ...
    public function get_resource($width = null, $height = null, $skip_cache = false, &$cache_status = null)
    {
        ...
        $scaled_path = sprintf(get_config_value('app.photos_scaled_dir'), $this->get_id(), $width, $height);
        ...
        if ($scale > 1.0 || (!$width && !$height)) {
            $scaled_path = sprintf(get_config_value('app.photos_scaled_dir'), $this->get_id(), $this->get('width'), $this->get('height'));
...
class DataModelPhotobook extends DataModel implements SearchProviderInterface
{
    ...
    public function get_random_book($count = 10)
    {
        $q = sprintf("
            ...
            ORDER BY
                c.date DESC
            LIMIT %d",
            get_policy($this)->getAccessLevel(),
            intval($count));
    ...
    public function get_photos_recursive(DataIterPhotobook $book, $max = 0, $random = false, $seed = null)
    {
        $query = sprintf("
            ...
            HAVING
                COUNT(f.id) > 0",
                $book->get_id(),
                get_policy($this)->getAccessLevel()); // BAD DEPENDENCY!
    ...
    public function delete(DataIter $iter)
    {
        ...
        $filled_in_filter = preg_replace('/%d/', $iter->get_id(), get_config_value('app.photos_scaled_dir'), 1);
