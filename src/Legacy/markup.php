...
function _markup_parse_mailinglist(&$markup, &$placeholders)
{
    ...
    while (preg_match('/\[mailinglist\]([^\[]+)\[\/mailinglist\]/i', $markup, $match)) {
        try {
            // TODO SFY: this does not work. At all.
            // {{ render(controller('App\\Controller\\MailingListsController::markup', {id: 47, referrer: app.request.requestUri})) }}
            $controller = new MailingListsController();
            $response = $controller->markup($match[1]);
            $content = $response->getContent();
        } catch (Exception $exception) {
            \Sentry\captureException($exception);
            $content = sprintf('<pre>%s</pre>', $exception->getMessage());
        }
